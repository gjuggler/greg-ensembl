    ...
    while (my $genome_db_id1 = shift @{$species_set}) {
      foreach my $genome_db_id2 (@{$species_set}) {
        ...
        if ($count) {
          ...
          print STDERR "method_link_species_set_id: ",$mlss->dbID,"(",$mlss->name,"); median: ",$median,"; 2\*median: ",2*$median;

          if($median >1.0) {
            print STDERR "  threshold exceeds 2.0 - too distant -> clear values\n";
            $median = 0.0;
          }
