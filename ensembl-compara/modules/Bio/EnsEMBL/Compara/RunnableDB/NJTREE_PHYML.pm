...
#    $taxon_tree->print_tree;
	...
	if ($potential_species > 1) {
	    my %species;
	    my @species_array = map {$_->taxon->name} @{$tree->get_all_leaves};  # Create an array of species names.
    ...
    $tree->store_tag("tree_species_sampling",$tree_species_sampling);
    $tree->store_tag("tree_species_num",$tree_species_num);
