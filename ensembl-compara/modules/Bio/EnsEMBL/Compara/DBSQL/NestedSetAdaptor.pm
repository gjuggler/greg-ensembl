...
# GJ 2009-04-25
sub fetch_all_subroots {
  my $self = shift;

  my $constraint = "WHERE root_id=parent_id AND root_id != 0";
  return $self->_generic_fetch($constraint);
}
...
sub fetch_root_by_node {
  ...
  # GJ 2008-12-20: warn about the indexing issue, and don't try to fetch if the indexing isn't there...
  unless ($node->left_index and $node->right_index) {
      warning("fetch_root_by_node needs left-right indexing to work correctly, and this doesn't seem to be the case for tree ".$node->newick_format."!");
      return undef;
  }
...
sub store {
  ...
  if(defined($node->parent)) {
    $sth->execute($node->parent->dbID, $node->name);
  } else {
...
sub sync_tree_leftright_index {
  ...
  foreach my $node ($tree_root->nodes) {
