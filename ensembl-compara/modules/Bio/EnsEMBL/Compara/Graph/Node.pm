...
sub find_node_by_name_regexp {
  ...
  foreach my $neighbor (@{$self->_walk_graph_until(-name => $name, -regexp => 1)}) {
    return $neighbor if($neighbor->name =~ /$name/);
  }
...
sub _walk_graph_until {
  ...
  if (scalar @args) {
    ($name, $node_id, $cache_nodes, $regexp) = 
       rearrange([qw(NAME NODE_ID CACHE_NODES REGEXP)], @args);
  }
  ...
  foreach my $neighbor (@{$self->neighbors}) {
    ...
    if (defined $name && defined $regexp && $neighbor->name =~ /$name/) {
      print STDERR "found " . $neighbor->name. "\n";
      last;
    }
    last if (defined $name && $name eq $neighbor->name);
    last if (defined $node_id && $node_id eq $neighbor->node_id);
    $neighbor->_walk_graph_until(-name => $name, -node_id => $node_id, -cache_nodes => $cache_nodes, -regexp => $regexp);
  }
