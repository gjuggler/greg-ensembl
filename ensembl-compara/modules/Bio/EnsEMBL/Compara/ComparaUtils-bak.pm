...
sub genomic_align_for_tree {
  ...
  my $url = 'mysql://anonymous@ensembldb.ensembl.org:5306/ensembl_compara_58';
  ...
  foreach my $exon (@exons[3..4]) {
    my $slice = $exon->slice;
    my $start = $exon->coding_region_start($tx);
    my $end = $exon->coding_region_end($tx);
    my $strand = $exon->strand;
    printf "%s %d-%d %s\n", $slice->seq_region_name,$start,$end,$exon->strand;
    my $sub_slice = $slice->sub_Slice($start,$end,$strand);
    
    #my $gat = $gat_a->fetch_all_by_MethodLinkSpeciesSet_Slice(
    #  $mlss,$sub_slice
    #  );
    
    my $gabs = $gab_a->fetch_all_by_MethodLinkSpeciesSet_Slice(
      $mlss,$sub_slice,
      );
    foreach my $gab (@{$gabs}) {
      #$gab->_print;
      my $sa = $gab->get_SimpleAlign("translated");
