...
\affil[1]{EMBL-EBI, Wellcome Trust Genome Campus, Hinxton, Cambridgeshire, CB10 1SD, United Kingdom}
\affil[*]{To whom correspondence should be addressed: Nick Goldman, Tel:~+44~(0)1223~492~530; Fax:~+44~(0)1223~494~468; E-mail:~goldman@ebi.ac.uk}
...
of positive selection. \tcr{Different aligners may also be designed
  ...
  the resulting alignment will be used. Here we focus on the sitewise
...
\tcr{In addition, we expect the protein structure and the
...
\begin{figure}[t]
\begin{center}
\hidegraphics{scale=0.8}{fig1.pdf}
\end{center}
...
\begin{figure}[t]
\begin{center}
\hidegraphics{scale=1.25}{fig2.pdf}
\end{center}
...
alignment. \tcrr{(Results for ProbCons, T-Coffee and \pranka
  ...
  aligners tested can be found in Supplementary Figure S1.)} For the
indel simulations, the indel rate here was held constant at 0.1 indel
event per substitution.
\begin{figure}[t]
\begin{center}
\hidegraphics{scale=1.25}{fig3.pdf}
\end{center}
...
\begin{figure*}[t]
\begin{center}
\hidegraphics{scale=0.8}{fig4.pdf}
\end{center}
...
\tcrr{(MAFFT and \pranka are omitted from Figure \ref{fig_4}A, B and
  ...
  results can be found in Supplementary Figure S1.)}  The results from
...
\begin{figure*}[t]
\begin{center}
\hidegraphics{scale=0.65}{fig5.pdf}
\end{center}
...
positives, we tested whether alignment filtering methods could reduce
...
range of indel rates and divergence levels, we calculated inferred
alignments using ClustalW and \prankc and applied four filtering
methods before performing the \sw analysis. Since we wished to
...
\ref{fig_5}. \tcrr{(Filtering results for \pranka, ProbCons and
  ...
  and \tprf values, can be found in Supplementary Figure S2.)}
...
\tcrr{Since the performance measurements shown in Figure \ref{fig_5}
  ...
  unfiltered alignments, we simulated additional datasets with
  ...
  \ref{fig_4}. These results are included in Supplementary Figure
  S1. Comparing absolute performance, we find that GUIDANCE filtering
...
significantly improved performance. Finally, we note that the
