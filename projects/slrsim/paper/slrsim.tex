...
%%\title{The effects of alignment error and alignment filtering on the \sw detection of positive selection in vertebrates}
...
space, and controls using the true alignment and an optimal filtering
...
\citep{Dessimoz2010Phylogenetic}. \textcolor{red}{A recent study suggested that the
benefit of alignment filtering (in terms of improved accuracy)
outweighs the cost (in terms of reduced power) when applied to
detecting positive selection, but the analysis was limited to a small
range of possible evolutionary scenarios, as discussed below
\citep{Privman2011Improving}.}  With the application of published
filtering methods to alignments before testing for positive selection
becoming standard practice
\citep{Studer2008Pervasive,Aguileta2009Rapidly}, \textcolor{red}{continued investigation into the}
potential benefits of alignment filtering to the detection of positive selection
...
approach \textcolor{red}{is modeled on that of} Anisimova et al.
...
positive or negative result. \textcolor{red}{As we simulate codon
  ...
  study of the effect of alignment
...
\textcolor{red}{Unlike Fletcher and Yang, here we focus on \sw
  ...
  alignment filtering methods on the \sw analysis. Thus, the
  biological hypothesis being investigated is different, and the
  addition of filtering methods adds another layer of complexity to
  our analysis.}
...
\textcolor{red}{We hypothesize that the divergence level and indel
  rate, two highly variable and important evolutionary factors, may
  strongly affect the performance of methods for alignment and
  detection of selection. Accordingly, our simulations encompass a
  wide range of biologically plausible indel rates and divergence
  levels while fixing other parameters at values typical to those
  encountered in the \sw analysis of vertebrate gene
  families. Although our main purpose is to understand the behaviour
  of various aligners and filters within our range of simulations, we
  will make use of the above-noted methodological similarities in
  order to highlight aspects of our results which agree with or
  contradict the literature where possible.}
...
ClustalW performing worst. \textcolor{red}{The same ranking of
  aligners with respect to detecting positive selection has been
  observed in a number of studies
  \citep{Fletcher2010Effect,Markova-Raina2011High,Privman2011Improving};
  our results corroborate these findings and provide evidence that
  this ranking may be applicable across a wide range of divergence
...
conditions. \textcolor{red}{Figure \ref{fig_4}B shows the FPR to be on
  the order of $5\times10^{-4}$ under those conditions, and we
  observed 63 false positives for the true alignment and 67 false
  positives for the \prankc alignment across all 150 replicates at the
  indel rate of 0.04 and MPL of 0.8}. A similar slight FPR elevation
was also observed at the same indel rate for the optimal, GUIDANCE and
T-Coffee filters.
...
species. \textcolor{red}{Thus, the use of unfiltered \prankc alignments would yield
largely equivalent performance to GUIDANCE-filtered alignments for detecting
\sw positive selection when analyzing protein-coding sequences across
a wide range of indel rates and commonly encountered sequence
divergence levels. On the other hand, GUIDANCE (when judiciously applied, including an upper limit on the amount of sequence data removed) never significantly reduced performance compared to the unfiltered alignments.} Finally, we note that the performance of the `optimal' filter on \prankc
alignments suggests that mild further improvements to filtering
strategies may be possible, but the potential for improvement is small
...
Fletcher and Yang \citeyearpar{Fletcher2010Effect}, Markova-Raina et al. \citeyearpar{Markova-Raina2011High}, and Privman et al. \citeyearpar{Privman2011Improving} regarding the
...
current filtering methods are \textcolor{red}{scarcely able to} improve the
...
divergence levels (whereas T-Coffee and Gblocks failed to improve upon
...
slight increase in power. On the other hand, judicious application of
GUIDANCE never significantly reduced power, so its use would not be
expected to yield worse results.

\textcolor{red}{Some of our results and conclusions differed from those of Privman et
...
env, respectively; E. Privman, personal communication) and were roughly
comparable in size and shape to our 17-taxon tree, we failed to find a
significant benefit for alignment filters at any MPL below 1.2 when
using \prankc alignments. Interestingly, the authors also found
Gblocks to be roughly comparable in performance to GUIDANCE, while we
...
on the accuracy and sensitivity of different aligners.}
