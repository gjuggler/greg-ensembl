...
sub create_simsets {
  ...
  sub indel_rates {
    ...
    foreach my $tr ( map { tree_param($_) }
      ( $fortyfourmammals, $tree_anisimova_bglobin, $tree_anisimova_artificial ) ) {
      foreach my $sp (@sim_params) {
        my $p = replace( $cur_params, $tr, $sp );
        Bio::EnsEMBL::Compara::ComparaUtils->hash_print($p);
        verify_params($p);
  ...
  sub all_trees {
    ...
    my @tree_params = map { tree_param($_) } (
      $ensembl, $ensembl_two, $twoxmammals, $tree_anisimova_artificial, $tree_anisimova_bglobin,
      $full, $primates, $glires, $nox, $fortyfourmammals
    );
  ...
  sub filtering_sim {
    ...
    foreach my $threshold ( 0, 2, 5, 7 ) {
      my $p =
        replace( filter_param('prank_treewise'), { alignment_score_threshold => $threshold } );
    ...
    my @aln_params =
      map { aln_param($_) } ( 'true', 'mcoffee', 'prank', 'prank_f', 'prank_codon_f' );
    foreach my $tr (@tree_params) {
  ...
  sub sitewise_sim {
    ...
    my @filter_params =
      map { filter_param($_) }
      ( 'none', 'prank_treewise', 'prank_mean', 'indelign', 'trimal', 'tcoffee' );
    my @aln_params =
      map { aln_param($_) } ( 'true', 'mcoffee', 'prank_f' );    #,'prank_f','prank_codon_f');
    foreach my $tr (@tree_params) {
      foreach my $aln (@aln_params) {
        foreach my $fp (@filter_params) {
          my $p = replace( $cur_params, $aln, $tr, $fp );
  ...
  sub ref_sim {
    ...
    my @species_params =
      map { species_param($_) } ( 'Human', 'Mouse', 'Xtropicalis', 'Stickleback' );
