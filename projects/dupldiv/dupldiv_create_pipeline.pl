...
sub mapping {
  ...
  my $cmd = "SELECT distinct(node_id) FROM sitewise_aln";
  my @nodes = _select_node_ids($cmd);
  my $job_params = {};
  _add_nodes_to_analysis($analysis_id,$job_params,\@nodes);  
}
...
sub node_sets {
  ...
  my $params = {
    flow_node_set => 2,  # Add small balanced duplications to the analysis.
    flow_parent_and_children => 1
  };
  _create_analysis($analysis_id,$logic_name,$module,$params,30,1);
