...
use base ('Bio::Greg::Hive::Process', 'Bio::Greg::StatsCollectionUtils', 'Bio::Greg::Hive::Align');
...
sub run {
  ...
  my $full_primate_tree = $self->good_primate_tree($full_tree,[9606,9593,9598,9600,9544,9483]);
  ...
  foreach my $non_gorilla ('chimpanzee','human') {
    ...
    if ($non_gorilla eq 'human') {
      ...
      my $aln_no_chimp = Bio::EnsEMBL::Compara::AlignUtils->remove_seq_from_aln($full_primate_aln,$chimp_member->stable_id);
      $aln = $aln_no_chimp;
      ...
      my $aln_no_human = Bio::EnsEMBL::Compara::AlignUtils->remove_seq_from_aln($full_primate_aln,$human_member->stable_id);
      $aln = $aln_no_human;
    ...
    $categories = {9604 => '1'};
    my $model_f = $self->calculate_branch_likelihood($full_primate_tree,$full_primate_aln,$categories);
    $self->store($model_f,$table,'f');  
    
    $categories = {9604 => '$1'};
    my $model_g = $self->calculate_branch_likelihood($full_primate_tree,$full_primate_aln,$categories);
    $self->store($model_g,$table,'g');

    $categories = {};
    my $model_h = $self->calculate_branch_likelihood($full_primate_tree,$full_primate_aln,$categories);
...
sub get_filtered_aln {
  ...
  $file = $self->save_aln($aln,{id=>$aln_id, filename => $aln_id."_orig",subfolder => 'branch_alns'});
  ...
  $file = $self->save_aln($aln,{id=>$aln_id, filename => $aln_id."_realign",subfolder => 'branch_alns'});
  ...
  if ($first_keeper && $second_keeper) {
    $aln = Bio::EnsEMBL::Compara::AlignUtils->remove_triple_mutated_codons($aln,$first_keeper->stable_id,$second_keeper->stable_id);
  }
  if ($first_keeper && $third_keeper) {
    $aln = Bio::EnsEMBL::Compara::AlignUtils->remove_triple_mutated_codons($aln,$first_keeper->stable_id,$third_keeper->stable_id);
  }
  if ($second_keeper && $third_keeper) {
    $aln = Bio::EnsEMBL::Compara::AlignUtils->remove_triple_mutated_codons($aln,$second_keeper->stable_id,$third_keeper->stable_id);
  }

  $file = $self->save_aln($aln,{id=>$aln_id, filename => $aln_id."_final",subfolder => 'branch_alns'});
  $self->param('filtered_aln_length',$aln->length);
  $self->param('filtered_aln_file',$file);

  Bio::EnsEMBL::Compara::AlignUtils->pretty_print($orig_aln,{width=>200}); # Debugging print the alignment.  
  Bio::EnsEMBL::Compara::AlignUtils->pretty_print($prank_aln,{width=>200}); # Debugging print the alignment.  
  Bio::EnsEMBL::Compara::AlignUtils->pretty_print($aln,{width=>200}); # Debugging print the alignment.  
