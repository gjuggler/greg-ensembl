...
sub fetch_input {
  ...
  $self->create_table_from_params( $self->compara_dba, $self->params->{counts_sites_table},
    $counts_sites_def );
  $self->create_table_from_params( $self->compara_dba, $self->params->{counts_genes_table},
    $counts_genes_def );
}
...
sub run {
  ...
  for ( my $i = 1 ; $i < $aln->length ; $i += 3 ) {
    ...
    $self->store_params_in_table( $self->compara_dba, $self->param('counts_sites_table'),
      $site_data );
  ...
  my $other_count = 0;
  foreach my $key ( keys %$codon_pattern_hashref ) {
    $other_count = $gene_data->{$key}
      if ( $key ne $most_frequent_pattern && $gene_data->{$key} > $other_count );
  }
  ...
  $self->store_params_in_table( $self->compara_dba, $self->param('counts_genes_table'),
    $gene_data );
}
...
sub get_genomic_coord {
  ...
  foreach my $leaf ( $tree->leaves ) {
    ...
    my @gc_arr     = $tscr->cdna2genomic( $cdna_start + $tscr->cdna_coding_start,
      $cdna_end + $tscr->cdna_coding_start + 2 );
