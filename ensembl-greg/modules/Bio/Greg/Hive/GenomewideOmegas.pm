...
sub run {
  ...
  my $tax_tree =
    ...
    'Chordata' );
  $tax_tree = Bio::EnsEMBL::Compara::TreeUtils->keep_members_by_method_call( $tax_tree, \@keepers,
    'taxon_id' );
...
sub fan_jobs {
  ...
  my @node_ids;
  my $sth =
    $self->dbc->prepare("select node_id from protein_tree_tag where tag='cc_root_Primates';");
  $sth->execute;
...
sub combine_all_alignments {
  ...
  foreach my $node_id (@node_ids) {
    ...
    last
      if ( defined $self->param('target_alignment_length')
...
sub load_and_save_aln {
  ...
  if ( $self->param('remove_funky_codons') == 1 ) {
    ...
    $filtered_aln =
      Bio::EnsEMBL::Compara::AlignUtils->remove_funky_stretches( $translated_aln, $first_keeper,
      $second_keeper, 3 );
    $filtered_aln =
      Bio::EnsEMBL::Compara::AlignUtils->remove_funky_stretches( $translated_aln, $second_keeper,
      $third_keeper, 3 );
...
#$filtered_aln = Bio::EnsEMBL::Compara::AlignUtils->remove_triple_mutated_codons($translated_aln,$first_keeper,$second_keeper);
  ...
  if ( $self->param('remove_gappy_columns') == 1 ) {
    ...
    print " >> Removing gaps...\n";
    $filtered_aln =
      Bio::EnsEMBL::Compara::AlignUtils->remove_gappy_columns_in_threes($filtered_aln);
