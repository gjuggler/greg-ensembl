...
sub run {
  ...
  my $nhx = Bio::EnsEMBL::Compara::ComparaUtils->get_genome_tree_nhx($compara,{labels => 'mnemonics'});
  print "$nhx\n";
    ...
    foreach my $homology (@homologies) {
      ...
      if ($desc eq 'ortholog_one2one') {
        ...
        if ($self->param('aln_type') eq 'cdna') {
          ...
          my ($genomic_cdna,$genomic_aa) = Bio::EnsEMBL::Compara::ComparaUtils->genomic_aln_for_member($compara,$member);
          $aln = $genomic_cdna;
...
#        $aln = $aln->remove_gaps(undef,1);
        ...
        my @substitution_run_lengths = Bio::EnsEMBL::Compara::AlignUtils->get_substitution_runs($aln);
 ...
 #         $self->hash_print($run_obj);
          ...
          my $ref_exon = $self->get_exon_from_position($ref_member,$run_obj->{ref_pos},\@ref_exons);
          my $other_exon = $self->get_exon_from_position($other_member,$run_obj->{other_pos},\@other_exons);

          my $cur_params = $self->params;
          $cur_params = $self->replace($cur_params,$run_obj,{
