...
sub fetch_input {
  ...
  $self->create_table_from_params( $dba, $params->{'collect_slrsim_stats_table'},
    $slrsim_stats_def );
}
...
sub get_data_for_node {
  ...
  eval {
    ...
    ( $tree, $sa_true, $cdna_true ) =
      Bio::EnsEMBL::Compara::ComparaUtils->tree_aln_cdna( $dba, $true_aln_params );
    ...
    #@aln_entropies = ();
    $sum_of_pairs_score =
      Bio::EnsEMBL::Compara::AlignUtils->sum_of_pairs_score( $sa_true, $sa_aln );
    $total_column_score =
      Bio::EnsEMBL::Compara::AlignUtils->total_column_score( $sa_true, $sa_aln );
...
#  die ("Reference was defined in params but not found in aln!") if ($reference_id ne '' && !defined $ref_seq);
  ...
  $cur_params->{column_entropy_mean_true} =
    Bio::EnsEMBL::Compara::AlignUtils->average_column_entropy($sa_true);
  $cur_params->{column_entropy_mean_aln} =
  ...
  $cur_params->{unfiltered_site_fraction} =
    $utils->unfiltered_site_count($sa_aln) / $utils->site_count($sa_aln);
  ...
  my $sth2 = $pta->prepare(
    "SELECT aln_position,omega,type,note,ncod,lrt_stat FROM $aln_table_name WHERE node_id=? AND parameter_set_id=?;"
  );
  ...
  for ( my $i = 1 ; $i <= length($nogaps) ; $i++ ) {
    ...
    $obj->{aln_lrt} = $aln_omegas->{$aln_col}->{'lrt_stat'} || 99;
