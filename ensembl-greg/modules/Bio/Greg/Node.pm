...
sub nodes {
  ...
  my @nodes = ();
  if ( $self->left ) {
    push @nodes, $self->left->nodes;
  }
  push @nodes, $self;
  if ( $self->right ) {
    push @nodes, $self->right->nodes;
  }
  return @nodes;
}
...
sub isLeaf {
  ...
  return 1 if ( !$self->left && !$self->right );
  return 0;
}
...
sub parseTree {
  ...
  my $class = "Bio::Greg::Node";

  while ( ( my $pair ) = ( $newick =~ /(\([\w#]+:\d+.\d+,[\w#]+:\d+.\d+\)#\d+#)/ ) ) {
    ( my $n1, my $d1, my $n2, my $d2, my $p ) =
      ( $pair =~ /\(([\w#]+):(\d+.\d+),([\w#]+):(\d+.\d+)\)(#\d+#)/ );
