...
sub new {
  ...
  my ( $aln, $tree, $st, $tempdir ) =
    $self->_rearrange( [qw(ALIGNMENT TREE SAVE_TEMPFILES TEMPDIR)], @args );
...
sub run {
  ...
  if ( ( defined $params{runmode} && ( $params{runmode} == 0 || $params{runmode} == 1 ) )
    && !$tree ) {
  ...
  my $tempseqfile = $self->_write_alignment(
    'phylip',
    -interleaved     => 0,
    -idlinebreak     => 1,
    -line_length     => 60,
...
sub set_parameter {
  ...
  if ( !defined $VALIDVALUES{$param} ) {
    $self->warn(
      "unknown parameter $param will not set unless you force by setting no_param_checks to true");
    return 0;
  }
